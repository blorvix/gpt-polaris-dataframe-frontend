...
const Avatar: React.FC<AvatarProps> = ({ role, lastMessageRole }) => {
  ...
  if (role === 'assistant' || role === 'function') {
    return (
      <img src={darkMode ? "/assets/openai-white-logomark.svg" : "/assets/openai-logomark.svg"} alt="Assistant avatar" width={40} height={40} className='p-2 rounded-md shadow-xl dark:bg-black bg-white' />
    );
  } else if (role === 'user') {
    return (
      <img src={darkMode ? "/assets/user-avatar-white.svg" : "/assets/user-avatar-white.svg"} alt="User avatar" width={40} height={40}
        className={'p-2 rounded-md shadow-xl dark:bg-fuchsia-700 bg-black'} />
    );
  } else if (role === 'function') {
    return (
      <img src={darkMode ? "/assets/function-avatar-white.svg" : "/assets/function-avatar.svg"} alt="Function avatar" width={30} height={30} />
    );
  } else {
    return null;
  }
