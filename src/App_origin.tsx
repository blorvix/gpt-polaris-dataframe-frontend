...
function App() {
  ...
  const sendMessage = async (userInput: string) => {
    try {
      ...
      const response = await fetch(`${Config.API_URL}/conversations/${currentConvId}/message`, {
        method: "POST",
        headers: {
  ...
  async function uploadFile(file: any) {
    ...
    try {
      const response = await fetch(`${Config.API_URL}/conversations/${currentConvId}/upload`, {
        method: "POST",
        body: formData,
      });
    ...
    const clickHandler = (event: any) => {
      ...
      if (element != null && element.tagName === 'A') {
        ...
        if (element.getAttribute("href").startsWith(`/download`)) {
          ...
          window.open(`${Config.API_URL}${element.getAttribute("href")}`);
        }        
      }
    };
